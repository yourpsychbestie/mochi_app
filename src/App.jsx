...
// ... (final del SVG de los pandas)
//     </defs>
//     <circle cx="50" cy="50" r="40" fill="pink" opacity="0.3" />
//   </svg>
// };
...
// ═══════════════════════════════════════════════
function App() {
  // ...todos los hooks y lógica aquí...
  // (mueve aquí todos los useState, useEffect, funciones, etc. que estaban fuera)

  // Fondo global morado claro 
}React.useEffect(() => {
    document.body.style.background = '#e6d6f7';
    return () => { document.body.style.background = null; };
  }, []);

  // Render condicionales de pantalla
  if (screen === "login") {
    return (
      <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "#e6d6f7" }}>
        <PandaSVG size={180} style={{ marginBottom: 32 }} />
        {/* ...aquí va el formulario/login original... */}
        <h2>Bienvenido a Mochi</h2>
        {/* ...puedes agregar aquí el resto del login... */}
      </div>
    );
...
// ...el resto del cuerpo de App y return final...
          ...
          <SectionErrorBoundary fallback={<div style={{ background: C.white, border: `1.5px solid ${C.border}`, borderRadius: 16, margin: 12, padding: 12, textAlign: "center", color: C.inkM, fontWeight: 700 }}>No se pudo cargar esta vista del jardín. Cambia de pestaña y vuelve a intentar.</div>}>
            ...
            <div onClick={onPet} style={{
              ...
              <div style={{ position: "relative", display: "inline-block" }}>
                ...
                {pandaBubble?.textA && (
                  <div style={{
                    position: "absolute", bottom: "90%", left: "-18px", maxWidth: 108, background: "white",
                ...
                {pandaBubble?.textB && (
                  <div style={{
                    position: "absolute", bottom: "84%", right: "-16px", maxWidth: 108, background: "white",
                    ...
                    {pandaBubble.textB}
                    <div style={{
                      position: "absolute", bottom: -8, right: 10, width: 0, height: 0,
          ...
          <div style={{ display: "flex", gap: 6, overflowX: "auto", padding: "10px 14px 6px" }}>
            {cats.map(c => (
              <div key={c.id} onClick={() => setShopTab(c.id)}
                style={{
                  background: shopTab === c.id ? C.dark : C.sandL, color: shopTab === c.id ? C.cream2 : C.inkM,
            ...
            {shopItems.map(item => {
              ...
              return (
                <div key={item.id} onClick={() => shopTab === "accesorios" ? onBuyAccessory(item) : onBuy(item)}
                  style={{
                    background: owned === true ? "#d4e8c4" : owned === "owned" ? C.cream : locked ? "#f0ede8" : C.sandL,
            ...
            <div style={{ display: "flex", gap: 4 }}>
              {[["ejerc", "🌿 Ejercicios"], ["lecciones", "📖 Lecciones"]].map(([id, label]) => (
                <div key={id} onClick={() => setEjTab(id)}
                  style={{
                    flex: 1, padding: "10px 0", textAlign: "center", fontFamily: "'Fredoka One',cursive",
                ...
                return DAILY_LESSONS.map(lesson => {
                  ...
                  return (
                    <div key={lesson.id} onClick={() => setOpenLesson(lesson)}
                      style={{
                        background: isToday && !iDone ? C.dark : C.white, borderRadius: 16,
...
`;
            ...
            {avgs.map(a => (
              <div key={a.id} style={{ marginBottom: 14 }}>
                ...
                <div style={{ display: "flex", gap: 4 }}>
                  {[1, 2, 3, 4, 5].map(i => (
                    <div key={i} style={{
                      flex: 1, height: 8, borderRadius: 50,
      ...
      <div style={{ minHeight: "100vh", background: C.sandL, display: "flex", flexDirection: "column", fontFamily: "'Nunito',sans-serif" }}>
        ...
        <div style={{ flex: 1, padding: "24px 20px" }}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            {[1, 2, 3, 4, 5].map(i => (
              <div key={i} onClick={() => setScore(i)}
                style={{
                  flex: 1, aspectRatio: "1", borderRadius: 14, display: "flex", flexDirection: "column",
          ...
          <button onClick={next} disabled={!canNext || saving}
            style={{
              width: "100%", background: canNext ? C.dark : C.sand, color: canNext ? C.cream2 : C.inkL,
  ...
  function LeccionDia({ lessonsDone, onComplete }) {
          ...
          <div style={{ padding: "16px 16px 0" }}>
            ...
            {!isDone ? (
              <button onClick={() => { onComplete(open.id); setReading(false); }}
                style={{
                  width: "100%", background: C.olive, color: C.cream2, border: "none", borderRadius: 14, padding: 16,
    ...
    function App() {
      ...
      // ...existing code before this comment...
    }
  }
        ...
        <div style={{ display: "flex", background: C.sandL, borderBottom: `1.5px solid ${C.border}` }}>
          {[["gratitud", "💛 Gratitud"], ["momentos", "✨ Momentos"]].map(([id, label]) => (
            <div key={id} onClick={() => setActiveTab(id)}
              style={{
                flex: 1, padding: "11px 0", textAlign: "center", fontFamily: "'Fredoka One',cursive",
            ...
            <>
              ...
              {!showForm ? (
                <button onClick={() => setShowForm(true)}
                  style={{
                    width: "100%", background: C.dark, color: C.cream2, border: "none", borderRadius: 14, padding: 14,
                  ...
                  <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
                    {[["A", nameA, "🐼"], ["B", nameB, "🐾"]].map(([w, name, emoji]) => (
                      <div key={w} onClick={() => setWho(w)}
                        style={{
                          flex: 1, padding: "10px 0", textAlign: "center", borderRadius: 12, cursor: "pointer",
              ...
              {gratitud.length === 0 ? (
                ...
                <div key={g.id || i} style={{
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                    <div style={{
                      background: "#fff0d0", borderRadius: 8, padding: "3px 10px",
              ...
              {!showMomentoForm ? (
                ...
                <div style={{ background: C.white, borderRadius: 18, padding: 16, marginBottom: 14, border: `1.5px solid ${C.border}`, boxShadow: `0 3px 0 ${C.border}` }}>
                  ...
                  <input value={momentoTitle} onChange={e => setMomentoTitle(e.target.value)}
                    ...
                    style={{
                      width: "100%", border: `2px solid ${C.border}`, borderRadius: 12, padding: "10px 12px",
